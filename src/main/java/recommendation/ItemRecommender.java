...
public class ItemRecommender {
    private static final int DEFAULT_GAME_LIMIT = 3; //3 game
    private static final int DEFAULT_PER_GAME_RECOMMENDATION_LIMIT = 10;//every game 10 videos
    private static final int DEFAULT_TOTAL_RECOMMENDATION_LIMIT = 20;//total 20 items
        ...
        for (Game game : topGames) {
            ...
            for (Item item : items) {
                if (recommendedItems.size() == DEFAULT_TOTAL_RECOMMENDATION_LIMIT) {
                    break outerfor; //break out for loop
                }
    ...
    private List<Item> recommendByFavoriteHistory(Set<String> favoritedItemIds, List<String> favoriteGameIds, ItemType type) throws RecommendationException {
        // Count the favorite game IDs from the database for the given user. E.g. if the favorited game ID list is ["1234", "2345", "2345", "3456"], the returned Map is {"1234": 1, "2345": 2, "3456": 1} //favitemID for dedup.
        Map<String, Long> favoriteGameIdByCount = favoriteGameIds.parallelStream()
                .collect(Collectors.groupingBy(Function.identity(), Collectors.counting()));
        //function.identity: sort by id, collections.counting: count first
        ...
        sortedFavoriteGameIdListByCount.sort((Map.Entry<String, Long> e1, Map.Entry<String, Long> e2) -> Long.compare(e2.getValue(), e1.getValue())); //sort by value

        if (sortedFavoriteGameIdListByCount.size() > DEFAULT_GAME_LIMIT) { //take the sublist
    ...
    public Map<String, List<Item>> recommendItemsByUser(String userId) throws RecommendationException {
        ...
        for (Map.Entry<String, List<String>> entry : favoriteGameIds.entrySet()) {
            //entry.getKey -> "video" entry.getValue -> GameIds
            if (entry.getValue().size() == 0) { //no fav gameIds, recomend by topgames
                TwitchClient client = new TwitchClient();
                List<Game> topGames;
                try {
